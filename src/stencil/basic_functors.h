...
namespace stencil {
    ...
    struct copy_functor : functor_base {
        ...
        void operator()(int index) const { dst[index] = src[index]; }
    };

    struct copyi_functor : functor_base {
        using functor_base::functor_base;

        void operator()(int index) const { dst[index] = src[index + istride]; }
    };

    struct copyj_functor : functor_base {
        using functor_base::functor_base;

        void operator()(int index) const { dst[index] = src[index + jstride]; }
    };

    struct copyk_functor : functor_base {
        using functor_base::functor_base;

        void operator()(int index) const { dst[index] = src[index + kstride]; }
    };

    struct avgi_functor : functor_base {
        using functor_base::functor_base;

        void operator()(int index) const { dst[index] = src[index - istride] + src[index + istride]; }
    };

    struct avgj_functor : functor_base {
        using functor_base::functor_base;

        void operator()(int index) const { dst[index] = src[index - jstride] + src[index + jstride]; }
    };

    struct avgk_functor : functor_base {
        using functor_base::functor_base;

        void operator()(int index) const { dst[index] = src[index - kstride] + src[index + kstride]; }
    };
