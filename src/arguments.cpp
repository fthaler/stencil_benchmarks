...
std::ostream &operator<<(std::ostream &out, const arguments_map &argsmap) {
    ...
    for (const auto &arg : argsmap)
        out << "    " << std::setw(name_maxl + 1) << std::left << (arg.first + ":") << "    " << arg.second
            << std::endl;
    return out;
}
...
void arguments::print_help() const {
    ...
    for (const auto &arg : m_args) {
        std::cout << "    " << std::setw(name_maxl + 2) << std::left << ("--" + arg.name) << "    ";
        if (!arg.default_value.empty())
            std::cout << std::setw(default_value_maxl + 11) << std::left << ("(default: " + arg.default_value + ")");
        else
    ...
    for (const auto &flag : m_flags) {
        std::cout << "    " << std::setw(name_maxl + 2) << std::left << ("--" + flag.name) << "    ";
        std::cout << std::setw(default_value_maxl + 11) << " ";
...
arguments_map arguments::parse(int argc, char **argv) const {
    ...
    if (!m_command_map.empty() && subcommand_argc == argc) {
        std::cerr << "Error: no valid sub-command for command '" << m_command_name << "'" << std::endl << std::endl;
        print_help();
        std::abort();
    }
    ...
    if (optind < subcommand_argc) {
        for (int i = optind; i < subcommand_argc; ++i)
            std::cerr << "Error: invalid " << m_subcommand_name << " '" << argv[i] << "'" << std::endl;
        std::abort();
    }

    if (!m_command_map.empty()) {
        argsmap.m_map[m_subcommand_name] = command;
        auto command_argsmap =
            m_command_map.find(command)->second->parse(argc - subcommand_argc, argv + subcommand_argc);
        for (const auto &a : command_argsmap.m_map)
            argsmap.m_map[a.first] = a.second;
        for (const auto &f : command_argsmap.m_flags)
            argsmap.m_flags.insert(f);
    }
